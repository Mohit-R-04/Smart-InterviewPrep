...
function App() {
    ...
    const schedule = useMemo(() => {
        ...
        // Try to get API key from environment first (developer's key), then localStorage (user's key)
        const geminiApiKey = import.meta.env.VITE_GEMINI_API_KEY || localStorage.getItem('gemini_api_key');

        if (geminiApiKey) {
            // Use AI scheduler
            generateAISchedule(problemsData, config, geminiApiKey)
                .then(result => {
                    if (result.aiGenerated) {
                        console.log('✅ Schedule generated by Gemini AI');
                        return result.schedule;
                    } else {
                        console.log('⚠️ Falling back to algorithmic scheduler');
