                ...
                return (
                    <div key={week.weekNum} className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md duration-300">
                        ...
                        <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                            {week.problems.map(p => (
                                <div key={p.id} className={`p-3 sm:p-4 transition-all duration-300 flex flex-col sm:flex-row items-start gap-3 sm:gap-4 group ${completed.has(p.id)
                                    ? 'bg-gray-100 dark:bg-slate-800/80 opacity-60 grayscale'
                                    : 'hover:bg-gray-50 dark:hover:bg-gray-700/30'
                                    }`}>
                                    {/* Mobile: Checkbox + Title Row */}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className={`text-sm sm:text-base font-semibold transition-colors ${completed.has(p.id)
                                            ...
                                            <div className="flex flex-wrap gap-2 sm:gap-3 text-xs items-center">
                                                <span className={`px-2 sm:px-2.5 py-0.5 rounded-md font-medium border text-[10px] sm:text-xs ${p.difficulty === 'Very Easy' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800' :
                                                ...
                                                <span className="flex items-center gap-1 text-gray-500 dark:text-gray-400 font-medium">
                                                    <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                    {p.duration}m
                                                </span>

                                                {p.topic && <span className="text-gray-400 dark:text-gray-500 font-medium hidden sm:inline">• {p.topic}</span>}
                                            </div>
                                        </div>
                                    </div>

                                    {/* 3. Meta Column (Video + Stats) - Responsive */}
                                    <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end ml-8 sm:ml-0">
                                        {/* Video Thumbnail */}
                                        {p.videoUrl && (
                                            <a
                                                href={p.videoUrl}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="group/video relative block w-20 h-12 sm:w-28 sm:h-16 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200 dark:border-gray-700 shadow-sm hover:ring-2 ring-blue-500 transition-all"
                                            ...
                                            >
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover/video:bg-transparent transition-colors">
                                                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-black/50 flex items-center justify-center backdrop-blur-sm group-hover/video:scale-110 transition-transform">
                                                        <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white fill-current ml-0.5" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                                    </div>
                                                </div>
                                            </a>
                                        )}
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            {/* Likes */}
                                            <div
                                                className="group/meta flex items-center gap-1 sm:gap-1.5 cursor-help"
                                                title="Number of people who like this question on LeetCode"
                                            >
                                                <span className="text-[9px] sm:text-[10px] font-medium text-gray-400 group-hover/meta:text-gray-600 dark:text-gray-500 dark:group-hover/meta:text-gray-300 transition-colors">
                                                    {p.likes > 1000 ? `${(p.likes / 1000).toFixed(1)}k` : p.likes || 0}
                                                </span>
                                                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-300 group-hover/meta:text-gray-500 dark:text-gray-600 dark:group-hover/meta:text-gray-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div
                                                className="group/meta flex items-center gap-1 sm:gap-1.5 cursor-help"
                                                title={`This question has been asked by ${p.company_count || 0} companies`}
                                            >
                                                <span className="text-[9px] sm:text-[10px] font-medium text-gray-400 group-hover/meta:text-gray-600 dark:text-gray-500 dark:group-hover/meta:text-gray-300 transition-colors">
                                                    {p.company_count || 0}
                                                </span>
