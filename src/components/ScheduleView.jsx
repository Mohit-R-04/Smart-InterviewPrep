            ...
            {schedule.map((week) => {
                ...
                return (
                    <div key={week.weekNum} className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md duration-300">
                        {/* Header - Responsive and Clickable */}
                        <button
                            onClick={() => toggleWeek(week.weekNum)}
                            className="w-full p-3 sm:p-5 bg-slate-100 dark:bg-slate-700/60 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 border-l-4 border-l-blue-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-left"
                        >
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white tracking-tight">Week {week.weekNum}</h3>
                                {collapsedWeeks.has(week.weekNum) ? (
                        ...
                        {/* List - Responsive - Only show if not collapsed */}
                        {!collapsedWeeks.has(week.weekNum) && (
